...
> 数据库命令题守则（1.0.0）：`code_completion_generator_1` 实现要求第 10 条与
...
> 1.0.0 实测通过；0.9.2「仅 `\$` 转义」与 0.9.4 heredoc 守则被证伪。1.6.19 起**外层包裹
> 由执行层统一决定**（模型只管语句内部的引号 / 转义 / 条数，不为此反复改写形态），另有
> 破坏性语句剔除与提交形态机器体检兜底（`agent/src/ai.mjs`，见 CHANGELOG 1.6.19–1.6.25）。
> 1.6.25 起包裹**按当轮实测的终端环境**决定：终端在数据库 REPL 里而命令写着 bash 形态时，
> 执行层只在序列开头插一条 `exit`（不改写你写的任何一条语句）；裸 `db.` 语句包成一次性客户端
> 调用、`use X` 的库名往下携带、`show …` 走 `echo … | mongo --quiet <db> --shell`（`show`/`use`
> 是 shell 交互内建、不是 JS，包进 `--eval` 必报 `SyntaxError`）。
...
- **加载前校验（fail-fast）**：`agent/src/capability-schema.mjs` 在能力首次加载时校验全部 JSON——必填字段（`id` / `formValue.prompt` / `paramsSchema`）、占位符 ⊆ `paramsSchema.properties`（占位符拼写错误此前会静默渲染为空串）、`required` ⊆ `properties`。手动校验跑 `cd agent && npm run caps-check`。
