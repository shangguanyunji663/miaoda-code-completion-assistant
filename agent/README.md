...
| `npm test` | 运行单测（54 项：核心纯函数 + 挑页链 + 能力配置校验 + 运行控制，Node 内置 `node:test`，零新增依赖） |
...
- **连续同类报错重载兜底依赖平台行为（1.3.0）**：同一异常连续 ≥3 轮自动重载题目页重取原始模板，前提是平台重载后编辑器恢复为原始模板（实测评测环境 EduCoder 如此）；若某平台重载后清空题目或进入新会话，重载会中断本题（重载上限 2 次，日志可见）。
