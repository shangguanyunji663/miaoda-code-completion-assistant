...
- **混合题**（如「先命令行插入文档、再代码栏写查询」）：先在命令行完成数据准备（至多两轮，输入期报错反思自愈），再落入代码分支常规作答；平台把代码栏内容当 shell 脚本执行时，数据库命令题按题面转义要求输出（`$`→`\$`，0.9.2 生成/反思守则）
...
- **混合题依赖终端可用**：混合题（先命令行数据准备、再代码栏作答）数据准备失败且反思自愈两轮用尽时按当前状态继续，评测环境可能已预置数据；平台把代码栏内容当 shell 脚本执行时，裸 `$`（未按题面转义）会触发 bash syntax error——生成/反思守则已覆盖（0.9.2），但遇到平台自定义执行形态仍需按实测调整。
