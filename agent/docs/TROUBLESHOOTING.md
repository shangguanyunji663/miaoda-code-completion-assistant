...
**解决**（1.6.23 起步，**1.6.24 定形**；`cmd-evidence.mjs` 零依赖纯函数 + `loop.mjs` 止损）：

- `detectMissingPremise(anchors, segment, { requiredTops })`：把"题面引用的源目录在**顶层目录**实测清单里**全部缺席**"分档为**前置数据缺失**。判据宁漏不误报——⓪ 参与判定的顶层必须**来自题面**、① 该顶层下有 ≥2 个题面点名的源目录、② 该顶层的 `find` 探针确已执行（段里认得到 `find T -maxdepth` 命令行片段）、③ 段中无 T 下任何路径行，四条全中才判；命中注入处置段（重做上一关备份 / 重置环境后按课程顺序重做）
- **1.6.24 的关键更正**：判据的输入原从终端回显抠（`extractMissingPaths`），而 xterm 按列硬换行会把 `/opt/collection_1` 劈成上行末的 `/opt` + 下行开头的 `/collection_1`——真机第 2 轮三条取证命令全是 `find /collection_1/…` 这种**幽灵顶层根**。后果有两头：碎片根不在 `/opt` 之下 ⇒ 条件①永不满足、`premiseStreak` 每轮归零、**2 连击永远凑不齐（止损形同不存在）**；而两个同前缀碎片自己构成一个顶层时 ⇒ 三条件全部"成立"、**把 `/opt/collection_1` 其实有数据的可解题判死**。修法是分工：**题面点名哪些目录由题面说了算（`requiredPathsFromProblem`，只认「编程要求」段——该词在页面文本里出现两次，须取最后一次；全文匹配会把相关知识的 `-o 指明到要导出的文件名` 误读成"要你自造数据"），目录里有什么由实测说了算**
- `premiseStreak` 改**锁存**语义：命中累加，**只有取证段真看到题面点名的源目录（正证据）才清零**——"本轮没判出来"与"本轮证明数据在"是两回事，不该互相抵消（旧版任一未命中即清零，正是被劈行碎片打败的地方）
- 止损阈值 `premiseLimit` 分档（`taskCreatesOwnData`）：题面「编程要求」要你自己产出数据（mongodump/插入/生成…）时给 2 轮，纯恢复题 **1 轮即停**。旧版把逃生口写成注入文本里的一句"若题面要求由你创建这些数据则忽略"——那是最弱的 prompt 层，现在做成判据
- `pathProbeCommands` 改**顶层根优先**（一条 `find /opt -maxdepth 4` 摊开整棵树），题面点名的顶层始终补进根清单 ⇒ 回显碎片不再占探针预算
- 课程模式 `solveBoard` 遇 `prerequisite-missing` 直接结束本板块并返回 `stoppedBy`，`courseLoop` 结尾点名"因前置数据缺失提前结束"的板块数（后续关卡依赖同一份前置数据会全部同因失败，不再逐关烧 `MAX_RETRY`）；混合题**数据准备**阶段同用该判定，命中即不再重试准备命令
...
2. **"前置数据缺失"是跨关卡的环境事实**：单题止损给 `prerequisite-missing`，课程模式遇它应结束本板块（后续关卡同因失败），把处置（补齐前置数据 / 重置环境后按课程顺序重做）显性化，而不是逐关烧 `MAX_RETRY`。
3. **锚点切片警惕前缀子串**：用"首个具体根"做 `lastIndexOf` 时，嵌套的具体根（`/opt/mongodb` vs `/opt/mongodb_1`）会让后一条命令偷走锚点，把要保留的那段截掉——锚点要取"不会互为前缀"的串（命令全文 / 带空格的顶层片段）。
4. **回显不是路径的可信来源**（1.6.24 教训）：终端文本会被 xterm 按列劈行，从回显抠出的"根"可能是根本不存在的幽灵目录，而它恰恰能满足所有"实测找不到"的判据。**凡是"题面要求哪些东西"这类问题，输入必须取题面**；"实际有什么"才取实测。
5. 取证结论模板**不许在证据为零时还给出出路**：旧判据在清单为空时仍写"要么用清单里的路径、要么改参数形态"，等于程序亲自把模型指示到"去改参数形态"（连烧三轮的推手）。清单为空必须翻向（与 format-probe「0 解 ⇒ 不是顺序问题是键集合问题」同一条设计）。

**已自动化（1.6.24）**：`requiredPathsFromProblem` / `topLevelRoots` / `taskCreatesOwnData` / `detectMissingPremise`（含 ⓪ 号闸与幽灵根反证）/ 锚点切片全部为纯函数并有真机 fixture 用例；本关在真机上验证过"第 1 轮即 `prerequisite-missing` 止损"与"补做第一关后判据自动不再命中"。仍**未走真机端到端**的是：`premiseLanded` 清零路径（需一次"中途补上数据"的真机序列）。

### C-24. 判据层和模型都说对了，执行层拿着上一轮的过期环境把命令改回错误形态

**现象**（2026-09-25 18:07 真机，`/tasks/XBLSCWNL/4881`，MongoDB setProfilingLevel 关）：两行相邻日志自相矛盾——

```
[loop] 终端环境识别：mongosh
[loop] bash 终端：3 条数据库 shell 语句已包成 mongo --eval
```

终端明明在 mongo REPL 里（`desc` 也明令"禁止再执行 mongo"），模型这一轮**也确实**按 REPL 写了 `use mydb` / `db.setProfilingLevel(1, 50)`；执行层却把它们包成 `mongo --eval '…'` 敲进 REPL，得到 `E QUERY [js] SyntaxError: missing ; before statement @(shell):1:8`（列 8 正是 `mongo ` 之后）。三轮逐字节同形，最后是 `output-unchanged` 把它叫停的——止损原因照旧泛化，看不出真凶在执行层。

**根因**

1. `loop.mjs` 里 `termEnv`（喂给 `applyCommandGuards` 的环境结论）是**首轮感知后跨轮复用**的——注释自己就这么写着。环境却会在一轮之内变：序列里一条 `mongo` 就把终端从 bash 变成 REPL。
2. 反思前 `detectTerminalEnv` 重新探测的结果（`envNow`）**只进了 prompt**，没回灌给执行层。于是同一个形态有两层各自决定：判据层 + 模型按 REPL 写，执行层按过期的 `bash` 改回去。
3. 同一场事故里还有两个连带缺陷：`use mydb` 被整行丢弃却不接住库名 ⇒ `--eval` 打在默认库上（模型连续三轮诊断出"未先切换到 mydb"，每轮都被执行层重新丢掉，**永远改不对**）；`show …` 被包进 `--eval` ⇒ 它是 shell 交互内建、不是 JS，bash 下同样必错（报 `@(shell eval)`，与 REPL 的 `@(shell)` 是两个不同位置标记，正好可用来分辨两类错误）。

**解决**（1.6.25）：

- 每轮**键入前重新实测** `detectTerminalEnv`，执行层用的就是当场的结论；环境变化打日志（`终端环境已变化：bash → mongosh`）。刻意不等提示符——认不出提示符是真实状态，`unknown` 时两层护栏都自动不生效（fail-open）。
- `planReplExit(env, cmds)`：实测在 REPL 里、而命令是 bash 形态（首词命中 `mongo`/`ls`/`cat`/`find`… 白名单）⇒ **只在序列开头插一条 `exit`**，不改写模型给的任何一条命令；REPL 内的合法语句（`use X` / `db.x()` / `SHOW …;` / `PING`）一律不动（表外不猜）。
- `wrapBareDbStatementsForShell` 三态分开：`use X` 的库名往下携带（优先于题面兜底）；有语句需改写时**丢弃光杆 `mongo`**（它就是把终端变成 REPL 的元凶，而无语句需改写时不动它）；`show …` 改用平台自己的 `echo 'show dbs' | mongo --quiet <db> --shell`。

**预防 / 纪律**

1. **跨轮复用"环境/状态"类结论，等于在用一个已经不存在的事实**。凡是会在执行过程中变化的东西（终端在哪层、服务起没起、当前库是谁），每次使用前就地重测；重测很便宜（读终端文本），错一次的代价是一整轮评测。
2. 「同一个形态只能有一层做决定」（1.6.19）还要补一句：**共用判据不只共用到"同一层规则"，还得共用到"同一时刻的事实"**。两层各拿一个时刻的结论，模型永远赢不了这场自我辩论。
3. 报错里的**位置标记是免费的分诊信息**：`@(shell)` = 敲进了 REPL，`@(shell eval)` = 语句本身不是合法 JS。这两种 SyntaxError 的处置完全相反（前者要 `exit`/剥壳，后者要换语句形态），判据该认，别让模型靠猜。
4. 执行层改写命令**必须留痕并进反思材料**：本例若没有那行 `bash 终端：3 条…已包成 mongo --eval` 日志，事后根本查不出是执行层改的。改写原因、丢弃原因（`sanitizeNote`）都要可读。

**已自动化（1.6.25）**：`planReplExit` 与 `wrapBareDbStatementsForShell` 三态全部为纯函数并用真机原始输入回放（`mongo ; use mydb ; db.setProfilingLevel(1, 50)` → 单条 `mongo --quiet mydb --eval 'db.setProfilingLevel(1, 50)'`）。**未走真机端到端**：`exit` 插入与 `--shell` 管道形态能否被平台评测接受。
