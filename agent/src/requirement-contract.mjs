...
/**
 * 叙述性条目判据（1.6.19，宁漏不误删）：这类行是**数据描述**或**操作位置提示**，
 * 不是"要实现的东西"，模型永远不可能给它一个实现落点。
 *
 * 为什么必须剔：2026-09-24 真机（mongo 查询关）的「编程要求」里混着
 * 「现有 person.json 文件内容如下：」「在右侧命令行进行操作：」「在右侧代码行 Begin-End 中
 * 编辑，如下：」三行叙述，契约层连它们一起切成 12 条，模型答了 7 条真要求里的 7 条，
 * 于是 `missing_item` 一次报满 12 条 ⇒ 触发"同一类问题打回一次仍是同一类 ⇒ 判定依据
 * 不可靠，停止打回"的止损，等于整个校验被噪声关掉，真正漏答的那条再没人拦。
 *
 * 注意与「引导语行必须单独成条」（见 splitItems 内注释）区分：`…如下：` 里点名了要写
 * 哪个函数的引导语**不能删**，所以判据只认"文件名/数据内容"与"右侧命令行/代码栏位置提示"
 * 这两种明确不含实现要求的形态。
 */
const NARRATIVE_ITEMS = [
  // 「现有 person.json 文件内容如下：」——题面给的数据，不是要做的动作
  /^(?:现有|已有|已存在|已知)[^。；]{0,40}?(?:文件|数据|内容)[^。；]{0,12}?[：:]/,
  // 「在右侧命令行进行操作：」「在右侧代码行 Begin-End 中编辑，如下：」——操作位置提示
  /^在[^。；]{0,24}?(?:命令行|代码行|代码栏|编辑器)[^。；]{0,24}?[：:]/,
];

function isNarrativeChunk(s) {
  const t = String(s ?? '').trim();
  // 注意：splitItems 合并续行时用的是空格，条目到这一步**已经是单行**，
  // 所以判据只能锚定在"开头就是这两种形态"上，不能要求整条以冒号结尾
  //（「现有…如下：」会把后面的表格行并进来，按结尾判就漏掉——1.6.19 单测抓到）。
  return NARRATIVE_ITEMS.some((re) => re.test(t));
}
