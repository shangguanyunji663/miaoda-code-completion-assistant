...
> **本分支的 agent 与 `feat/2-c-web-service` 同源**：下面 1.6.22–1.6.25 四条条目按源分支原文收录
> （标题里的分支名是该批变更的**发生地**），同步到本分支时未改写 agent 行为；桌面壳自身变更见
> [`../desktop/CHANGELOG.md`](../desktop/CHANGELOG.md)。

## [1.6.25] - 2026-09-25（分支 feat/2-c-web-service）

**执行层不再拿着过期的环境结论，把模型和判据都做对的结果改回错误形态。** 起因（2026-09-25 18:07 真机，MongoDB setProfilingLevel 关）：相邻两行日志自相矛盾——`[loop] 终端环境识别：mongosh` 紧跟 `[loop] bash 终端：3 条数据库 shell 语句已包成 mongo --eval`。判据层已正确识别终端在 mongo REPL 里、`desc` 明令"禁止再执行 mongo"、模型也照做给了 REPL 形态；执行层却复用首轮 `envGen`（`termEnv` 的注释自己写着「首轮感知后跨轮复用」），把 REPL 形态又包成 `mongo --eval '…'` 敲进 REPL ⇒ `SyntaxError: missing ; before statement @(shell):1:8`（列 8 正是 `mongo ` 之后），三轮逐字节同形直到 `output-unchanged` 止损。这是 1.6.19「同一个形态只能有一层做决定」的第二次实例，且这次被覆盖的一方是**正确的**。

### Fixed

- **`termEnv` 跨轮复用**：命令行分支改为**每轮键入前重新实测** `detectTerminalEnv`，环境变化时打日志（`终端环境已变化：bash → mongosh`）。刻意不等提示符——认不出提示符是真实状态，`unknown` 时两层护栏都不生效（fail-open）。
- **`use X` 的库名被丢掉却不接住**：旧版整行丢弃 `use mydb`，库名只从题面 `parseImportTarget` 兜底，于是 `--eval` 打在默认库上——模型连续三轮诊断出"未先切换到 mydb"，每轮都被执行层重新丢掉，**永远改不对**。现在序列里的 `use X` 往下携带（优先于题面兜底）。
- **光杆 `mongo` 会把终端就地变成 REPL**：同序列里后续语句既然被改写成一次性调用，这条进入命令就既多余又有害（正是上面那个 `@(shell):1:8` 的来源）。现在仅在确实有语句要被改写时丢弃它，并记进 `sanitizeNote`；无语句可改写时不动（模型可能就是要进 REPL）。
- **`show …` 包进 `--eval` 在 bash 里同样必错**：`show` 是 shell 的交互内建而非 JS。真机 `mongo --eval "show dbs"` 报的是 `@(shell eval):1:5`，与 REPL 的 `@(shell):1:8` 是两个不同位置——这两个标记正好把两类错误分开。现改用平台自己 `step2/Testdb.sh` 的写法：`echo 'show dbs' | mongo --quiet <db> --shell`。

### Added

- **`planReplExit(env, cmds)`**（`ai.mjs`，零依赖纯函数）：实测在数据库 REPL 里、而命令序列写的是 bash 形态（首词命中 `mongo`/`ls`/`cat`/`find`… 白名单）⇒ **只在序列开头插一条 `exit`**，不改写模型给的任何一条命令（保留原语义）。REPL 内的合法语句（`use X` / `db.x()` / `SHOW …;` / `PING`）一律不动——表外不猜。

### Verified

- `npm test` 239/239（新增 6 条：`show` 换形态、`use` 库名往下带、光杆 `mongo` 丢弃与"无语句可包时不动"的反证、`planReplExit` 三档）；`npm run lint` / `caps-check` 干净。
- 用真机 18:06:55 那一轮的原始输入回放：`mongo ; use mydb ; db.setProfilingLevel(1, 50)` → 现产出单条 `mongo --quiet mydb --eval 'db.setProfilingLevel(1, 50)'`（库名正确、终端留在 bash）；第 2 轮在实测 mongosh 下 REPL 形态原样键入。
- **验证边界**：以上为纯函数与真机输入回放，**未走真机端到端**（本关已可解，需真机再跑一次确认 `exit` 插入与 `--shell` 管道形态被平台接受）。

## [1.6.24] - 2026-09-25（分支 feat/2-c-web-service）

**1.6.23 那条「前置数据缺失」判据，在它自己引用的那份真机日志上跑不出来。** 本轮把它接回题面、补上防误报闸，并堵掉用户点名的另一件事：输入期已经报错还照着把整串命令敲完。

### Fixed

- **判据的 roots 来源（阻断级）**：`detectMissingPremise` 原先吃 `extractMissingPaths` 从终端回显抠出的根，而 xterm 按列硬换行会把 `/opt/collection_1` 劈成上行末的 `/opt` + 下行开头的 `/collection_1`——真机第 2 轮的三条取证命令 `find /collection_1/person`、`find /collection_1`、`find /collection_2/student` 就是证据。碎片不在 `/opt` 之下 ⇒ 条件①（≥2 个具体根位于同一顶层 T 下）永不满足 ⇒ `premiseStreak` 每轮归零、2 连击永远凑不齐，止损形同不存在（第 1 轮明明已实测出 `/opt` 是空的）。现在**题面点名哪些目录由题面说了算**：新增 `requiredPathsFromProblem`（只认「编程要求」段）+ `topLevelRoots`，判据吃题面路径。
- **幽灵顶层根的误报**：同一次劈行让 `/collection_1` 这种**根本不存在的顶层目录**进了根清单，而三条旧判据对它全部"成立"（幽灵根当然 find 不到，而 find 不到的命令正是我们自己生成的）——实测 `missing:true, top:'/collection_1'`，据此会把一道 `/opt/collection_1` 其实有数据的可解题判死、并连带让课程模式退出板块。新增 ⓪ 号条件：参与判定的顶层必须来自题面。
- **探针预算被碎片占满**：`pathProbeCommands` 改为**顶层根优先**（一条 `find /opt -maxdepth 4` 就能摊开整棵树），题面点名的顶层还始终补进根清单。
- **`premiseStreak` 语义**：由"任一未命中即清零"改为**命中即累加、只有取证段真看到题面点名的源目录（正证据）才清零**——"本轮没判出来"与"本轮证明数据在"是两回事，不该互相抵消。
- **差异定位把结构性缺失降格成格式问题**：逐行配对在"实际比预期整块少 7 行"时必然错位，真机 13 处差异里有 3 处被标成 `[WHITESPACE] 按预期那一行的空白照抄`——而 `show dbs` 的列宽由**库的数量**决定，照抄空白是不可执行指令，真因是六个库一个都没恢复出来。现在 `summarize` 先点名结构性缺行，空白类带上"连带结果"限定。
- **兜底归因的自查翻车**：`absentPathsFromEcho` 初版在"命令只有一个路径"时无条件兜底，回放真机 17 条时把第 1 条 `find /opt -maxdepth 3 -type d`（正常打印一个 `/opt`、零报错）判成"`/opt` 不存在"，于是后面 10 条全被跳过。兜底现在硬性要求**存在不存在信号行**。

### Added

- **跳过必败命令**（`absentPathsFromEcho` / `isDeadCommand` 纯函数 + `runTerminalCommands` 接线）：路径已被序列内前一条命令实测为不存在、且剩余命令里没有任何创建动作 ⇒ 不再键入。回放真机第 1 轮：17 条 → 键入 12 条（含全部取证命令）、跳过 5 条 `mongorestore`。跳过清单单独成段进反思材料（"把同一批路径再写一遍、只换工具参数，不改变结果"）。
- **止损阈值分档** `taskCreatesOwnData`：题面「编程要求」要你自造数据（mongodump/插入/生成…）时给 2 轮，纯恢复题 1 轮即停。旧版把这个逃生口写成注入文本里的一句"若题面要求由你创建这些数据则忽略"（prompt 层），现在做成判据。**只看「编程要求」段**：真机题干的相关科学知识里有 `-o 指明到要导出的文件名`，按全文匹配会把纯恢复题误判成自造数据题。
- `prerequisite-missing` 上抛到课程模式汇总（`solveBoard` 返回 `stoppedBy`，结尾点名"因前置数据缺失提前结束"的板块数）；混合题**数据准备**分支也接上了 `premiseMissing`（旧版只取 `.note`，明知造不出数据还要烧第二个准备轮次）。

### Verified

- `npm test` 234/234（新增 9 条：题面路径提取的「编程要求」段作用域、幽灵根 ⓪ 号闸、劈行两种形态的归因、必败跳过与"先 mkdir 再 restore"反证、无信号行不得判缺席、结构性缺行优先与"无缺行时不得无病呻吟"）；`npm run lint` / `caps-check` 干净。
- 判据链路用本次真机数据回放：轮 1 命中缺失、轮 2（回显全是碎片）仍命中、`/opt` 真有数据时不命中且 `premiseLanded` 翻回计数。
- **验证边界**：跳过必败命令与 `prerequisite-missing` 止损的**端到端真机行为仍未验证**（需要受控浏览器 + 一个源数据缺失的环境）；本轮验证的是纯函数判据与真机序列回放。
- 顺带修正上一轮的一处指认：`collectTestSetDetails` 的预期/实际切分**没有**问题（实测 `expected` 就是带 8 条数据的参考块、`actual` 就是全 0 那份），"参考答案混进实际段"系误判，已撤回。

## [1.6.23] - 2026-09-25（分支 feat/2-c-web-service）

**「前置数据缺失」确定性判定 + 止损——题面引用的源数据目录在环境里根本不存在时，不再反复重放同一批注定失败的命令。** 起因（2026-09-25 真机，mongorestore 恢复关）：题面要求"把 `/opt/mongodb*` 的备份恢复到 mytest1~4"，但环境里 `/opt` 是**空目录**（`find /opt -maxdepth 4` 只回 `/opt`），上一关的备份产物没落到这个环境。历轮回显的 `stat /opt/mongodb: no such file` 只证明了"这些名字不存在"，旧的取证结论模板说"用清单里出现过的路径、或按题面改参数形态"，可清单里**什么源数据都没有**——模型无路可走就回退去改 mongorestore 参数，连续 3 轮逐字节相同才撞上「改了等于没改」止损，且止损原因是泛化的 `output-unchanged`，看不出真实卡点。

### Added

- **`cmd-evidence.mjs` 新增 `detectMissingPremise(roots, segment)`**（零依赖纯函数）：把"题面引用的源目录在**顶层目录**实测清单里**全部缺席**"从"猜错路径名"这一档里分出来，升级成"前置数据缺失"。判据宁漏不误报：① 取证根里同时有顶层根 T（单段，如 `/opt`）与 ≥2 个位于 T 之下的具体缺失根；② T 的 `find` 探针确实执行过（段里认得到 `find T -maxdepth` 命令行片段——被 xterm 按列劈行即判"证据不足"不结论）；③ 段中没有任何一行是 T 下的路径（T 为空目录，或 T 本身也不存在，find 回显 `find: '/opt': No such file`）。三条全中才判缺失，任一不满足 fail-open。命中时返回注入反思材料的中文处置段（"改写本关命令不可能让数据凭空出现；处置：重做上一关备份 / 重置环境后按课程顺序重做"，并留"若题面要求你创建这些数据则忽略本节"的逃生口）
- **`loop.mjs` 命令行分支新增 `prerequisite-missing` 止损**：`probeMissingPaths` 改回 `{ note, premiseMissing }`；`premiseStreak` 连续 2 轮命中即终止本题（`reason: 'prerequisite-missing'`），不再等满 `outputFingerprint` 的 3 轮——源数据不在环境里，再烧评测额度全是白试。混合题准备阶段同用 `.note` 注入
- **`solveBoard`（课程模式）遇 `prerequisite-missing` 即结束本板块**：本关的源数据缺失时，后续关卡若依赖同一份前置数据会全部同因失败，不再逐关烧 `MAX_RETRY`，把处置（补齐前置数据）显性化到日志

### Fixed

- **`probeMissingPaths` 取证段锚点被嵌套具体根"偷走"**：旧版用 `find ${roots[0]}`（首个具体根，如 `/opt/mongodb`）做 `lastIndexOf` 锚点，而 `/opt/mongodb` 是**后一条**探针命令 `find /opt/mongodb_1 …` 的前缀子串，`lastIndexOf` 命中的是后一条，把**顶层目录那段**（T 的空清单恰是"前置数据缺失"判据的唯一证据）整个截掉。现锚点优先级：① 第一条探针**命令全文** → ② 顶层根命令片段（`find /opt -maxdepth`，其后是空格不是 `/`，不会撞上具体根命令行）→ ③ 旧锚点兜底；xterm 劈行认不到就退化为尾部 1600 字符

### Verified

- `npm test` 225/225（新增 7 条 `detectMissingPremise` 用例：顶层为空/顶层本身不存在/清单含源目录/非题面目录/仅 1 个具体根/T 探针未执行/空输入，全部取自 2026-09-25 真机回显）
- 验证边界：`probeMissingPaths` 的**取证执行**（真实 xterm 敲 `find` + 读回显）与 `prerequisite-missing` 止损**未走真机**（需受控浏览器 + 一个"源数据缺失"的真机环境）；纯函数判据与锚点切片已单测覆盖

## [1.6.22] - 2026-09-25（分支 feat/5-f-electron-desktop）

**网页工作台前端视觉重构 + 结果结构化渲染——纯展示层，`agent/src` 与 `shared/` 零改动，API 与按钮文案不变。** 起因：桌面版把本工作台作为主界面后发现"界面太粗糙"（结果全是裸 JSON 堆砌），在桌面包 0.2.0 一并优化。所有 `web-server.mjs` 端点、轮询节奏（状态 5s / 日志 1.5s）、按钮文案（探测页面 / 解当前题 / 停止做题 / 保存）与页内确认层语义原样保留。

### Changed

- **`public/index.html` 整页重构**（仍为单文件原生前端、无构建链、无外部资源——离线可用）：
  - **布局**：单列堆叠 → 双栏（左：运行状态 / 解题操作 / 快速上手；右：执行结果 / 运行日志），窄屏（<1020px）自动折回单栏；深色"控制台"视觉体系（墨蓝底 + 细点阵纹理 + 玻璃卡片 + 靛蓝品牌色 + 入场渐显），品牌标识（圆角方块+对勾）与桌面版配置窗口、托盘图标统一
  - **结果区**：不再裸 JSON 堆砌——探测结果渲染为「目标页/题型/编辑器/题干长度/可点按钮/候选页」键值清单 + 可展开题干预览；解题结果渲染为判定横幅（✓ 评测通过 / ✕ 未通过（原因+轮数）/ ■ 已手动停止 / 请求失败）+ 判定细节 + 评测输出折叠块。**原始 JSON 始终保留**在「原始 JSON」折叠块内（排查依赖不变），并加「复制原始 JSON」按钮
  - **日志区**：新增「自动滚动」开关（手动向上翻看旧日志时不再被顶走）与「清屏」按钮（只清显示、服务端环形缓冲不变）；增量提示改为「+N 条 / 无新增」
  - **状态区**：AI 配置 / 浏览器连接改为顶栏状态胶囊（带色点）+ 状态卡内明细；运行阶段指示带旋转 spinner

### Verified

- 本地 mock（伪造 `/api/status`、`/api/probe`、`/api/solve`、`/api/logs`、`/api/stop`、`/api/config`）下浏览器逐态走查：初始 / 探测 / 通过 / 未通过 / 已停止 / 请求失败 / 确认弹层 / 窄屏折叠，渲染分支全部覆盖；**未走**真 agent 实跑（需受控浏览器，桌面版既有验证边界不变）
- `agent/src` 与 `shared/` 未改动，`npm test` 结果与 1.6.21 相同
...
- **`resetTaskEnv(page)`**（`act.mjs`）：点 `[title="工具栏"]`（图标 `icon-gongjuxiang`，用户口中的"公文包"）→ 「功能」菜单 → 文本锚定点「重置环境」→ 有确认弹窗就点确定。**选择器全部用 title / 文本锚定**，不写带 hash 的 CSS module 类名（本项目一贯纪律）
- **接进"数据未落库"的恢复路径**（混合题前置）：`verifyDataLanded` 实测到目标集合 0 条时，先用「重置环境」把容器文件系统与数据库恢复初始（**刷新题目页做不到这件事**——真机里被覆盖的 `/home/example/person.json` 与空库都救不回来），再重做一轮数据准备并复核条数；重置失败则明说"按现状继续，不要自造数据"
- `shared/platform-facts.json` 新增 `task_env` 段：重置入口（含 DOM 特征与菜单全貌）、"重置能恢复环境"、"刷新页面不恢复环境" 三条实测 + evidence + date

### Verified

- `npm test` **186/186**、lint、format:check、caps-check 全绿（点击类函数无纯逻辑可单测，靠只读 DOM 探针钉住结构）
- 探针实测到的结构：`div.action-bar___bbigU` 下 `a[title="工具栏"]` / `a[title="全屏"]`；命令行 tab 项内另有 `aside[title="复制"]`
- **未重跑真机**（重置是破坏性动作，本轮只实现与接线，未在真机上触发）

### Notes

- **重置会清掉容器内的数据与终端会话**（已导入的数据会丢失）⇒ 只在"环境确实坏了"时调用；本版本仅在"实测条数为 0 且已完成一轮数据准备"这一个条件下触发
- 未做：`代码文件` 标签下工具栏第三个按钮（用户截图里的另一个入口）未单独适配——目前统一走 `[title="工具栏"]`；`db.<库名>.<集合名>` 自动改写仍未做
