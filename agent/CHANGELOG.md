...
- **推理模型思考默认关闭（根治思考拖垮/耗尽类问题）**：请求附 `chat_template_kwargs.enable_thinking=false`（端点参数探测实证：思考 119→**0** 字）——常规运维/编程任务不需要深度思考，此前实测思考 43173 字仍耗尽预算、错误码位（丰→\u5346）致数据不匹配；`AI_ENABLE_THINKING=1` 可开回。真机复刻失败场景验证：同一生成 8.0s 完成、明文中文正确
- **预算回归务实值（思考关闭后输出可控）**：命令行生成/反思、代码生成/反思 32768→8192，quiz 批量 16384→4096，quiz 单题 16384→1024，意图路由 8192→512——预算回归"输出兜底"本位（均为预期输出的 3~5 倍余量）
