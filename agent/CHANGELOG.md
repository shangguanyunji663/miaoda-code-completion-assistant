...
## [1.7.1] - 2026-09-25（分支 feat/4-e-oneclick-launcher）

**把源分支 `feat/2-c-web-service` 的 1.6.22–1.6.25 全量同步进来。** 本分支只加了一键入口、没改 agent 逻辑，所以同学在一键入口上做的题仍然会遇到源分支修掉的那几类问题；本次同步后两支的解题行为重新一致。这些文件在同步前与源分支逐字节相同，无冲突。

### Added

- **`planReplExit(env, cmds)`**（`ai.mjs`，纯函数）：实测在数据库 REPL 里、而命令序列写的是 bash 形态时，**只在序列开头插一条 `exit`**，不改写模型给的任何一条命令；REPL 内的合法语句（`use X` / `db.x()` / `SHOW …;` / `PING`）一律不动。
- **必败命令跳过**（`cmd-evidence.mjs` 的 `absentPathsFromEcho` / `isDeadCommand` + `runTerminalCommands` 接线）：某路径已由序列内前一条命令实测为不存在、且剩余命令里没有任何创建动作 ⇒ 不再键入，跳过清单单独成段进反思材料。只读取证（`silent`）不参与跳过。
- **题面路径驱动的前置数据缺失判定**：`requiredPathsFromProblem`（只认「编程要求」段）、`topLevelRoots`、`taskCreatesOwnData`（止损阈值分档）。

### Fixed

- **执行层拿过期环境改写命令**（源分支 1.6.25 的根因修复）：命令行分支改为**每轮键入前重新实测** `detectTerminalEnv`。旧版跨轮复用首轮结论，出现过「判据层已识别 mongosh、模型也按 REPL 写了语句，执行层却按 bash 把它们包成 `mongo --eval` 敲进 REPL ⇒ `SyntaxError @(shell):1:8`，三轮逐字节同形烧到止损」。
- **前置数据缺失判据真能触发**（1.6.24）：判据输入从"回显抠路径"改为"题面点名路径"——xterm 按列劈行会把 `/opt/collection_1` 切成上行 `/opt` + 下行 `/collection_1`，产生**幽灵顶层根**：既让止损条件永不满足（真机 2 连击凑不齐），又会对着幽灵根三条判据全中而把可解题判死。新增 ⓪ 号闸、`premiseStreak` 锁存语义、探针预算顶层优先。
- **`wrapBareDbStatementsForShell` 三态分开**：`use X` 的库名往下携带（旧版丢掉 `use` 又不接库名 ⇒ 语句打在默认库上，模型连续三轮诊断对却改不对）；有语句需改写时丢弃光杆 `mongo`（它就是把终端变成 REPL 的元凶）；`show …` 改用平台自己的 `echo … | mongo --quiet <db> --shell`（`show`/`use` 是交互内建、不是 JS，包进 `--eval` 必报 `SyntaxError`）。
- **差异定位不再把结构性缺行降格成"照抄空白"**（`output-diff.mjs`）：逐行配对在"实际比预期整块少 N 行"时必然错位，而 `show dbs` 的列宽由成员数量决定——真机把三行标成"按预期空白照抄"，那是对模型不可执行的指令。
- **课程模式止损上抛**：`solveBoard` 返回 `stoppedBy`，`courseLoop` 结尾点名"因前置数据缺失提前结束"的板块数；混合题数据准备分支接上 `premiseMissing`（旧版只取 `.note`，明知造不出数据还要烧第二个准备轮次）。
- 网页工作台重做状态面板与探测/求解结果渲染（源分支 1.6.22）——一键入口起的正是这个工作台。

### Verified

- `npm test` 239/239、`npm run lint`、`npm run caps-check` 全绿；`.md` 内部链接零断链。
- **验证边界**：与源分支一致——纯函数判据 + 真机原始输入回放，`exit` 插入与 `--shell` 管道形态、`prerequisite-missing` 止损均**未在本分支跑过真机端到端**。
- 分支特有修正：根 README 的徽章（原写 1.6.21，本分支实际已是 1.7.x）与「分支说明」段（原文整段抄自源分支、把自己写成"基于本分支切出的减负分支"）按本分支事实重写。
