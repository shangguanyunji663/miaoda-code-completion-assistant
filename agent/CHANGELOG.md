...
- **【平台适配逻辑通用化 1.3.0，回灌自 `feat/2-c-web-service`】四级挑页链**（`browser.mjs` 的 `pickTargetPageWithMeta` + 新增 `task-url.mjs`）：① `TARGET_URL_HINT` →② 内置题目页 URL 形状 →③ 内容级兜底（强代码编辑器 / 评测面板标记 / 评测按钮）→④ 首个非空白页；**URL 层多命中报错**防解错页。修复背景：真机开着多个标签页时旧版两档挑页会选错页。
- **编辑器写入策略（`perceive.mjs` 的 `writeEditorCode`，+44/−4）**：按编辑器**分版本**选 API 形态——Monaco/CodeMirror5 优先 `model.setValue`/`cm.setValue`（触发内容变化事件、字节级精确；实测 Monaco 的 formatOnPaste/autoIndent 会把 `insertText` 进来的预缩进 Python 逐行重排致评测不匹配），API 不可用再回退「点击 → Ctrl+A → Delete → insertText」；写入后**回读验证**。另含 `readEvalPanel` / `waitForEditor` / `classifyTask` 的判据微调。
- **模板拼接以原始模板为权威 + 兜底直通（`ai.mjs`，+53/−23）**：`spliceIntoTemplate` 仅替换 Begin/End 标记间代码体（按序逐对、保留行首缩进）；**标记不成对或 AI 输出为完整代码（≥2 个模块级语句）时整体直通**——绝不按不可信标记丢函数；新增 `emptyMarkerBlocks` / `collectMarkerPairs` / `withTemplateLeadingDecl`，替换旧的 `findMarkerLine`。
- **重载兜底 + 草稿判据（`loop.mjs`）**：同一类报错 ≥3 轮自动重载题目页重取平台原始模板（上限 2 次）；重载后先跑 `looksLikeOwnDraft` 判据——EduCoder 系平台持久化草稿且无「恢复初始代码」，重载只能拿回上一轮自提交的草稿，命中即保留原存档并停用兜底。另含 `slimForReflection`（喂给反思的题干裁剪）与 `formatInputErrors`（输入期报错归并）。
- **按钮文案点击兜底（`act.mjs` 的 `clickByKeywords`，+54/−32）** 与 `ensureTaskPage` / `waitEvalResult` / `collectTestSetDetails` / `runTerminalCommands` 的判据加固：平台按钮文案与评测面板结构在不同课程/版本下不一致，改为关键字多候选 + 可见性判据，而非单一文案匹配。
- **模型端点侧的思考预算控制（`config.mjs` + `ai.mjs`）**：`AI_THINKING_STALL_MS`（停滞检测 45s）、`AI_THINKING_MAX_CHARS`（思考字数配额 24000）、`AI_FIRST_PASS_THINKING`（首轮关思考，时间优先）、`AI_THINKING_CAP_MS` 默认 20s → 120s；`maxTokens` 8192 → 32768。
- **单测 +19/+31 项**：新增 `click-fallback` / `reload-draft-guard` / `slim-for-reflection` / `pick-target` 与扩充后的 `ai.test.mjs`（模板拼接、标记兜底、成功判定等纯函数契约）。
- **本分支不含 `control.mjs`（「停止做题」）**：移植时已把 `ai.mjs`(5) / `act.mjs`(3) / `loop.mjs`(16 处 `checkStop` + `beginRun`/`endRun` + `StopRequested` 分支) 的停止检查点剥离，行为等价（无 UI 可请求停止 ⇒ 检查点永不命中）。`web-server.mjs` / `mcp-server.mjs` 维持各自分支特性。
