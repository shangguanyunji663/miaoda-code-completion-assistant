...
一个「壳」，**不复制、不改动 `agent/` 的逻辑**（0.2.0 起主窗口加载的 agent 工作台前端 `agent/public/index.html` 属纯展示层重构，随桌面包一并优化；`agent/src` 与 `shared/` 相对源分支零改动——桌面包按白名单打包它们，故本分支 agent 与源分支同源、当前 1.6.25）。定位与分支 `feat/4-e-oneclick-launcher`（一键 bat）同目标、不同形态：同学拿到手是**安装版桌面程序**（图标 / 托盘 / 无黑窗口）。给同学的分步操作手册见仓库根《[桌面版使用手册.md](../桌面版使用手册.md)》；分支定位、取舍与验证边界见《[桌面版说明.md](../桌面版说明.md)》；本文件面向开发者，讲构建与打包细节。变更历史见 [CHANGELOG.md](CHANGELOG.md)。
...
- **构建已实测**（2026-09-25，本机 Windows）：0.1.0 `npm install` + `npm run dist` 走通，产出便携版 exe（76MB，未签名属预期）；`dist/win-unpacked/resources/` 下 `agent/`（含 node_modules/playwright-core）与 `shared/` 布局核验正确，**包内确认无 `.env.local`**（密钥不进分发物）。0.2.0 已重新打包实测：产出 `Miaoda Setup 0.2.0.exe`（NSIS）+ `Miaoda-portable-0.2.0.exe`（便携），资源内 `agent/public/index.html` 确为新版、`shared/` 正确、包内仍无 `.env.local`；旧 0.1.0 便携版已删除。NSIS 安装版目标与便携版同一管线。
...
- 托盘/窗口图标是**代码生成**的 32×32 32bpp BMP（靛蓝渐变圆角方块 + 白色对勾，4× 超采样抗锯齿；仓库不放二进制资产）；要换正式多尺寸图标，把 `build/icon.ico` 放进 desktop/ 即自动生效。
- Web 工作台本身的能力边界（仅绑 127.0.0.1、solve 会真实提交等）见 `agent/README.md`「网页工作台」节，桌面版不改变其中任何一条；**0.2.0 起工作台前端视觉（`agent/public/index.html`）属 agent 展示层改动，agent 端点 / API / 行为不变**。
