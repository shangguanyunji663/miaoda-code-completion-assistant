...
一个「壳」，**不复制、不改动 `agent/` 的任何代码**。定位与分支 `feat/4-e-oneclick-launcher`（一键 bat）同目标、不同形态：同学拿到手是**安装版桌面程序**（图标 / 托盘 / 无黑窗口）。用户视角的功能说明、与一键 bat 的取舍及验证边界见仓库根《[桌面版说明.md](../桌面版说明.md)》；本文件面向开发者，讲构建与打包细节。变更历史见 [CHANGELOG.md](CHANGELOG.md)。
