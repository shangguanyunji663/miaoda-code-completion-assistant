...
- `output-diff.mjs` — 输出差异定位器（1.6.4，纯函数）：`splitExpectedActual` 取面板/明细里最后一对「预期输出 / 实际输出」，`diffOutputs` 逐行对齐并**分类**差异（`ORDER_ONLY` 元素同集合仅顺序不同 / `WHITESPACE` 只差空白 / `MISSING_OR_EXTRA` 缺行或多行 / `VALUE_DIFF` 给第一个不同字符的位置与上下文），并对顺序差异做**机械的排序假设排除**——预期各行是否整体等于字典升序、是否整体等于降序，两者都不成立时直接给出"任何排序策略都无法解释，疑似 Python 2 的 set 迭代顺序被 list() 转出"的结论。`describeOutputDiff` 渲染成注入反思的中文段（无差异/无结构时返回空串）。存在理由：真机见过模型在两坨 2074 字符文本里找了 2.4 万字思考仍没看出"部分行要升序、部分行要降序"这种一眼可判的矛盾
