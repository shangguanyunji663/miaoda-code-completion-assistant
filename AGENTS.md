...
- `act.mjs` — 执行层：写入代码、勾选选项、点击评测、等待结果、翻页、课程导航（退出/返回/开始学习/跳转检测）、切换「命令行/代码文件」工作区标签、向 xterm 终端逐条键入命令、客户端可用性实测（`probeTerminalClients`）、shell 护栏清洗（`sanitizeShellSubmission`）。1.5.0 两条硬约束：① `waitEvalResult(page, ms, {codeUnchanged})` **只在提交文本与上次送进评测的相同时**才允许采信"面板与点击前一致"，且等待预算按面板「本关最大执行时间」动态抬高，未观测到变化就返回带 `STALE_EVAL_PREFIX` 的文本（`parsePlatformMaxSeconds` / `evalDeadlineAt` 为可单测纯函数）；② `clickByKeywords` 返回 `exists`，"按钮点不动"（上一轮评测仍在跑）与"页面没有按钮"分开记，`clickEval` 重扫窗口默认 150s 且每轮过 `checkStop`
...
- `output-diff.mjs` — 输出差异定位器（1.6.4，纯函数）：`splitExpectedActual` 取面板/明细里最后一对「预期输出 / 实际输出」，`diffOutputs` 逐行对齐并**分类**差异（`ORDER_ONLY` 元素同集合仅顺序不同 / `WHITESPACE` 只差空白 / `MISSING_OR_EXTRA` 缺行或多行 / `DICT_ORDER` 键值一一对应只有键序不同（→ 结论是 Redis 写入顺序问题，须用 OrderedDict/逐字段 hset，**改字典字面量书写顺序是空操作**）/ `VALUE_DIFF` 给第一个不同字符的位置与上下文），并对顺序差异做**机械的排序假设排除**——预期各行是否整体等于字典升序、是否整体等于降序，两者都不成立时直接给出"任何排序策略都无法解释，疑似 Python 2 的 set 迭代顺序被 list() 转出"的结论。标记写法兼容「预期输出/预期：/【预期】」；`diffSkipReason` 在未启用时把原因打进日志（静默失效必须可见）。`describeOutputDiff` 渲染成注入反思的中文段（无差异/无结构时返回空串）。存在理由：真机见过模型在两坨 2074 字符文本里找了 2.4 万字思考仍没看出"部分行要升序、部分行要降序"这种一眼可判的矛盾
- `loop.mjs` — 编排：生成 → 评测 → 反思循环；`solveOnce` 整体包 `beginRun`/`endRun`（`endRun` 在 `finally`，成功/失败/被停止三态运行态都不悬空）并在各阶段埋 `checkStop` 检查点（意图判定、切工作区、每轮重试开头、**语法守卫修正**、**提交评测前**、反思调用前、重载题目页前）；`watchLoop` 常驻监听；`liteLoop` 刷新触发监听；`courseLoop` 课程自动驾驶；混合题先在命令行做数据准备（输入期报错反思自愈）再落代码分支；代码反思教训链（`lessons` 逐轮注入防横跳）与连续同类报错重载兜底（同一异常 ≥3 轮自动重载题目页重取原始模板，上限 2 次；1.4.3 起重载后先做 `looksLikeOwnDraft` 草稿判据，本平台拿回草稿即不覆盖存档并停用兜底——`slimForReflection` / `looksLikeOwnDraft` 为可单测的纯函数导出）。1.5.0 新增：提交流水线抽出为 `finalizeSubmission`（拼接 → 空区域打点 → shell 护栏 → echo 包裹，反思/守卫修正共用）、写入前过 `py2-guard`、`verdictOf()` 统一处理"本轮结果未确认"的陈旧面板（这类轮次的诊断**不进教训链**）、`lastEvaluatedCode`/`lastEvaluatedCommands` 记账供 `waitEvalResult` 判同错复现。1.5.1 新增：`outputFingerprint` / `pickEvalEvidence` 实际输出指纹——同一输出连续 3 轮即判定"改了等于没改"并终止本题（`output-unchanged`）
