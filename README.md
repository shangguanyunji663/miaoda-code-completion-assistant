...
[![Version](https://img.shields.io/badge/version-1.6.25-2f6fed?style=flat-square)](agent/CHANGELOG.md)
...
> **📖 分支说明**：本分支（`feat/5-f-electron-desktop`）= `feat/2-c-web-service` 基线 + **Electron 安装版桌面程序**（桌面包 **0.2.0**，agent 与源分支同源、当前 **1.6.25**）：同学拿到手是带图标与托盘的**安装版/便携版**，双击即起「受控 Edge + 网页工作台」，无需命令行。说明见《[桌面版说明.md](桌面版说明.md)》、分步操作见《[桌面版使用手册.md](桌面版使用手册.md)》、构建与打包见 [`desktop/README.md`](desktop/README.md)。**`agent/src` 与 `shared/` 相对源分支不另改逻辑**（桌面版只是壳，用 Electron 自带 Node 子进程跑 agent 既有入口）。姊妹分支：`feat/2-c-web-service`（网页工作台本体，agent 1.6.25）、`feat/4-e-oneclick-launcher`（一键 bat 入口，agent 1.7.1，说明见其分支上的《一键启动说明.md》）、`feat/3-d-capability-guard`（能力配置自动校验）、`feat/1-ad-mcp-server`（MCP 出口，让 AI 编程助手替你做题，**该分支有新增依赖**）。
...
1. **Prompt 单一数据源**——AI 行为只定义在 `shared/capabilities/*.json`（7 个能力配置），Agent 读取后渲染 `{{input.xxx}}` 占位符，代码内不复制 prompt，杜绝两份漂移；能力首次加载时由 `capability-schema.mjs` 预检（占位符 ⊆ `paramsSchema.properties`），“占位符拼错 → 静默渲染空串”从运行期问题变为启动即报。
...
6. **平台事实单一数据源**——实测的平台事实（Python 2 运行时、旧版 redis-py 的 `zadd` 实参顺序、评测方独占 stdout 等）只维护在 `shared/platform-facts.json`，由 `ai.mjs` 的 `buildPlatformFactsBlock()` 统一注入每个能力 prompt 末尾（`readCapability` 是唯一注入点，新增能力自动继承）；`npm run caps-check` 校验其结构——事实段必须带 `evidence` 与 `date`，推断/待验证项隔离在 `unknowns`。
...
cli.mjs（入口：解析命令，分发到对应模式）
  ...
  │    ├─ ai.mjs（生成：意图路由 / 生成 / 反思 / 结果判定；加载前预检能力配置 + 注入平台事实）
  ...
  ├─ control.mjs（运行控制层：停止请求 + 运行态快照，供 loop / act / ai 埋检查点）
...
│   │   ├── output-diff.test.mjs    # 输出差异定位：差异分类 / 排序假设排除 / 服务端报错解读
│   │   ├── format-probe.test.mjs   # 容器格式反解探针（只读纯计算 / 白名单 / 可照抄结论）
│   │   ├── candidate-rank.test.mjs # 多候选择优（闸门评分 + 多数派投票）
│   │   ├── requirement-contract.test.mjs # 题面契约切条与对齐校验（叙述行不进清单）
│   │   ├── eval-context-contract.test.mjs # 「现有输入」文件覆盖 + 语句内单引号两道机器判据
│   │   ├── command-lines-fence.test.mjs # 命令解析：围栏逐行剥离 + 提示符剥离
│   │   ├── terminal-env.test.mjs   # 终端环境识别等待与 unknown 三态描述
│   │   ├── terminal-input-window.test.mjs # 输入期报错窗口（按内容求重叠，C-19）
│   │   ├── cmd-evidence.test.mjs   # 回显失败两档判据 + 祖先目录取证
│   │   ├── db-submission-guards.test.mjs # 数据库提交护栏（破坏性语句 / 条数 / 形态 / echo 包裹）
...
├── desktop/                        # Electron 桌面壳（本分支 0.2.0）：构建见 desktop/README.md，用户说明见《桌面版说明.md》/《桌面版使用手册.md》
...
| [`桌面版说明.md`](桌面版说明.md) | **本分支专用**：Electron 桌面版的用户说明、与一键 bat 的取舍、验证边界 |
| [`桌面版使用手册.md`](桌面版使用手册.md) | **本分支专用**：给同学的分步操作手册（装 → 首次配置 → 每天怎么用 → 排障 FAQ → 隐私边界） |
| [`desktop/README.md`](desktop/README.md) | 桌面版构建与打包细节（开发者向） |
