...
| 环节 | 行为 |
|---|---|
| 感知 | 提取题干，按题干内容判别任务类型（**代码题 / 命令行题 / 混合题 / 选择 / 填空**），定位编辑器、终端与按钮 |
| 作答 | 混合题先命令行插入题面文档（评测环境共享终端数据库）再写代码栏；代码题优先经编辑器 API 写入（触发平台自动保存，API 不可用才退回键盘输入）；命令行题逐条键入 xterm 终端并回车；选择题/填空题批量勾选填入 |
